...
func test_init_reviews_counts_grants() -> void:
	...
	assert_eq(rt.reviews_left, 3, "base 1 + Spare Review (+1) + Review Master (+1)")
...
func test_captains_call_grants_spare_reviews() -> void:
	...
	assert_eq(rt.reviews_left, 2, "Captain's Call grants +1 review (base 1 + 1)")
	...
	assert_eq(rt.reviews_left, 1, "failed review consumed")
