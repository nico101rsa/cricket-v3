...
func test_pure_bowler_bats_tail() -> void:
	...
	var pos := InningsResolver.player_position(_attrs(12.5, 12.5, 50.0, 50.0), itun)
	assert_gte(pos, 7, "pure bowler bats the tail")

func test_even_build_bats_middle() -> void:
	var pos := InningsResolver.player_position(_attrs(31.25, 31.25, 31.25, 31.25), itun)
	assert_eq(pos, 5, "even build -> #5")

func test_position_clamped_to_range() -> void:
	var pos := InningsResolver.player_position(_attrs(50.0, 50.0, 6.25, 6.25), itun)
...
func test_pure_batter_bowls_no_overs() -> void:
	...
	assert_eq(InningsResolver.player_overs(_attrs(50.0, 50.0, 12.5, 12.5), itun), 0, "specialist batter doesn't bowl")

func test_even_build_bowls_full_quota() -> void:
	# 5/5/5/5: share 0.5 -> 13*0.5 - 2.6 = 3.9 -> round 4 (a real all-rounder bowls out)
	assert_eq(InningsResolver.player_overs(_attrs(31.25, 31.25, 31.25, 31.25), itun), 4, "all-rounder bowls the full 4-over quota (authenticity)")

func test_batting_ish_build_bowls_somewhat() -> void:
	# 7/7/3/3: share 0.3 -> 13*0.3 - 2.6 = 1.3 -> round 1 (bowls somewhat, not zero — DA5)
	assert_eq(InningsResolver.player_overs(_attrs(43.75, 43.75, 18.75, 18.75), itun), 1, "a 7/7 bowls somewhat (~1 over), not zero")

func test_pure_bowler_bowls_full_quota() -> void:
	# 2/2/8/8: share 0.8 -> 8*0.8 - 2.5 = 3.9 -> round 4 (== max)
	assert_eq(InningsResolver.player_overs(_attrs(12.5, 12.5, 50.0, 50.0), itun), 4, "specialist bowler bowls the full quota")

func test_overs_monotonic_in_bowling_share() -> void:
	var batter := InningsResolver.player_overs(_attrs(50.0, 50.0, 12.5, 12.5), itun)
	var mid := InningsResolver.player_overs(_attrs(31.25, 31.25, 31.25, 31.25), itun)
	var bowler := InningsResolver.player_overs(_attrs(12.5, 12.5, 50.0, 50.0), itun)
...
func test_player_bowler_off_matches_baseline() -> void:
	...
	var base := InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(777))
	var off := InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(777), 0, null, null, null, 0, 0, 0)
...
func _avg_conceded(p_attack: float, p_control: float, n: int) -> float:
	...
	for seed_value in range(1, n + 1):
		total += InningsResolver.simulate_innings(
			null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(seed_value),
			0, null, null, null, p_attack, p_control, 4).total
	return float(total) / n

func test_strong_player_bowler_concedes_fewer_runs() -> void:
	var weak := _avg_conceded(12.5, 12.5, 80)
	var strong := _avg_conceded(50.0, 50.0, 80)
...
func test_innings_deterministic_with_player_bowler() -> void:
	var r1 := InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(99), 0, null, null, null, 50.0, 50.0, 4)
	var r2 := InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(99), 0, null, null, null, 50.0, 50.0, 4)
...
func test_player_bowling_figures_zero_when_not_bowling() -> void:
	var r := InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(5))
...
func test_player_bowling_figures_bounds_and_consistency() -> void:
	...
	for seed_value in range(1, 40):
		var r := InningsResolver.simulate_innings(
			null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(seed_value), 0, null, null, null, 43.75, 43.75, 4)
...
func _avg_player_economy(p_attack: float, p_control: float, n: int) -> float:
	var runs := 0
	var balls := 0
	for seed_value in range(1, n + 1):
		var r := InningsResolver.simulate_innings(
			null, 31.25, 31.25, 31.25, tuning, itun, _make_rng(seed_value), 0, null, null, null, p_attack, p_control, 4)
...
func test_strong_player_bowler_has_better_economy() -> void:
	var weak := _avg_player_economy(12.5, 12.5, 60)
	var strong := _avg_player_economy(50.0, 50.0, 60)
...
func _sim(attrs: Attributes, partner: float, oa: float, oc: float, seed_value: int) -> InningsResult:
...
func test_same_seed_gives_identical_innings() -> void:
	...
	var r1 := _sim(a, 31.25, 31.25, 31.25, 4242)
	var r2 := _sim(a, 31.25, 31.25, 31.25, 4242)
...
func test_termination_bounds() -> void:
	for seed_value in range(1, 60):
		var r := _sim(_attrs(31.25, 31.25, 31.25, 31.25), 31.25, 31.25, 31.25, seed_value)
...
func test_all_out_stops_immediately() -> void:
	...
	for seed_value in range(1, 40):
		var r := _sim(_attrs(12.5, 12.5, 50.0, 50.0), 12.5, 87.5, 87.5, seed_value)
		if r.wickets == 10:
			seen_all_out = true
...
func _avg_player_balls(attrs: Attributes, n: int) -> float:
	var total := 0
	for seed_value in range(1, n + 1):
		total += _sim(attrs, 31.25, 31.25, 31.25, seed_value).player_line()["balls"]
	return float(total) / n

func test_participation_responds_to_build() -> void:
	var batter_balls := _avg_player_balls(_attrs(50.0, 50.0, 12.5, 12.5), 60)
	var bowler_balls := _avg_player_balls(_attrs(12.5, 12.5, 50.0, 50.0), 60)
	assert_gt(batter_balls, bowler_balls, "a batter-build Player faces more balls than a bowler-build Player")

func _avg_total(attrs: Attributes, partner: float, oa: float, oc: float, it: InningsTuning, n: int) -> float:
	var total := 0
	for seed_value in range(1, n + 1):
...
func test_weakening_tail_drags_total_down() -> void:
	...
	flat.tail_floor = 1.0
	flat.tail_slope = 0.0
	var with_tail := _avg_total(_attrs(31.25, 31.25, 31.25, 31.25), 31.25, 31.25, 31.25, itun, 80)
	var flat_total := _avg_total(_attrs(31.25, 31.25, 31.25, 31.25), 31.25, 31.25, 31.25, flat, 80)
...
func test_even_contest_total_in_sane_t20_band() -> void:
	var avg := _avg_total(_attrs(31.25, 31.25, 31.25, 31.25), 31.25, 31.25, 31.25, itun, 200)
...
func test_player_line_integrity() -> void:
	var r := _sim(_attrs(50.0, 50.0, 12.5, 12.5), 31.25, 31.25, 31.25, 77)
...
func test_strike_rotation_and_ball_accounting() -> void:
	var r := _sim(_attrs(31.25, 31.25, 31.25, 31.25), 31.25, 31.25, 31.25, 2024)
...
func test_null_player_builds_all_derived() -> void:
	var r := InningsResolver.simulate_innings(null, 37.5, 31.25, 31.25, tuning, itun, _make_rng(7))
...
func test_default_target_matches_explicit_zero() -> void:
	...
	var a := _attrs(31.25, 31.25, 31.25, 31.25)
	var r1 := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _make_rng(99))
	var r2 := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _make_rng(99), 0)
...
func test_chase_stops_when_target_reached() -> void:
	...
	for sv in range(1, 40):
		var r := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _make_rng(sv), 10)
		if r.total >= 10:
...
func test_null_plan_matches_balanced_baseline() -> void:
	...
	var a := _attrs(31.25, 31.25, 31.25, 31.25)
	var base := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(7), 0, null)
	var balanced := InningsResolver.simulate_innings(
		a, 31.25, 31.25, 31.25, tuning, itun, _rng(7), 0, IntentPlan.balanced())
...
func test_innings_deterministic_with_plan() -> void:
	...
	var r1 := InningsResolver.simulate_innings(
		a, 31.25, 31.25, 31.25, tuning, itun, _rng(99), 0, IntentPlan.textbook())
...
func test_aggressive_outscores_and_outdies_defensive() -> void:
	...
	for sv in range(1, 41):
		var ra := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(sv), 0, agg)
		var rd := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(sv), 0, def)
		agg_runs += ra.total
		agg_wkts += ra.wickets
		def_runs += rd.total
		def_wkts += rd.wickets
...
func test_null_bowling_matches_scalar_baseline() -> void:
	...
	var a := _attrs(31.25, 31.25, 31.25, 31.25)
	var base := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(7))
	var explicit := InningsResolver.simulate_innings(
		a, 31.25, 31.25, 31.25, tuning, itun, _rng(7), 0, null, null, null)
...
func test_innings_deterministic_with_rotation() -> void:
	var a := _attrs(31.25, 31.25, 31.25, 31.25)
	var ba := BowlingAttack.new(31.25, 31.25)
	var r1 := InningsResolver.simulate_innings(
		a, 31.25, 31.25, 31.25, tuning, itun, _rng(99), 0, null, ba, BowlingPlan.textbook())
	var r2 := InningsResolver.simulate_innings(
		a, 31.25, 31.25, 31.25, tuning, itun, _rng(99), 0, null, ba, BowlingPlan.textbook())
...
func test_pace_takes_more_wickets_at_higher_run_rate_than_spin() -> void:
	...
	var spin_wkts := 0
	for sv in range(1, 41):
		var rp := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(sv), 0, null, ba, pace)
		var rs := InningsResolver.simulate_innings(a, 31.25, 31.25, 31.25, tuning, itun, _rng(sv), 0, null, ba, spin)
		pace_runs += rp.total
		pace_balls += rp.balls
...
func test_build_batters_clone_path_unchanged_when_no_roster() -> void:
	var batters := InningsResolver._build_batters(null, 31.25, itun)
	assert_eq(batters.size(), 11, "11 batters")
	assert_eq(batters[0]["power"], 31.25, "opener clone == partner_batting * factor(1)=1.0")
...
func test_build_batters_uses_real_roster_individuals() -> void:
	...
	assert_eq(batters[10]["power"], 6.25, "tail is a real BOWLER (power 6.25, steep tail BB5)")
	for b in batters:
...
func _e2_innings(plan: IntentPlan, seed_v: int, target: int) -> InningsResult:
	var rng := RandomNumberGenerator.new()
	rng.seed = seed_v
	return InningsResolver.simulate_innings(null, 31.25, 31.25, 31.25, tuning, itun, rng, target, plan)
