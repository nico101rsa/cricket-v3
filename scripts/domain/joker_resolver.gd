...
static func roll_mults(jokers: Array, player_is_batting: bool, intent: int, ball: int, field_mode: int = FieldPlan.Mode.NEUTRAL, bowl_intent: int = -1, bowler_type: int = -1, is_chase: bool = false) -> Vector2:
	...
	for j in jokers:
		if j.sets_field != -1 and j.matches(player_is_batting, intent, ball, eff_field, bowl_intent, bowler_type, is_chase):
	...
	for j in jokers:
		...
		if j.matches(player_is_batting, intent, ball, eff_field, bowl_intent, bowler_type, is_chase):
