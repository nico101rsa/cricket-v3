...
static func roll_mults(jokers: Array, player_is_batting: bool, intent: int, ball: int, field_mode: int = FieldPlan.Mode.NEUTRAL) -> Vector2:
	var wm := 1.0
	var rm := 1.0
	for j in jokers:
		if j.matches(player_is_batting, intent, ball, field_mode):
