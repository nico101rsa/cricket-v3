...
| **BB4** | Player-bowled overs: no phase bonus, `bowler_kind = -1` to resolve_ball; C2d `bowler_type` joker gating untouched. | Player bowling kind is still deferred (player-as-bowler D4); don't invent one implicitly. |
