...
**WS9 — proportional strength noise (Nico's ruling 2026-06-14).** The star→league-finish check (`tools/standings_correlation.gd`) found the bottom tour broken: at Club Flat & Warm the correlation was **r=0.07** (stars meaningless) and the weakest team won the league **22%** — the fixed ±6.25 per-Season jitter swamped the tiny low-tour star spread. Fix: noise is now **proportional** (`TourDistribution.noise_frac=0.7`, continuous ± noise_frac×spread; supersedes the fixed `noise`/`noise_step`). Result: the **NPC teams now sort cleanly by stars at every tour** (Club Flat & Warm slots 1-7: 2.0★→6.3rd … 4.5★→2.9th, monotonic). Env held (mean 155, in band); seeded-season pin re-captured (pos 1→2, total 224→189). 547 green.

**KNOWN — deferred to its own rung (the structural over-boost):** the overall correlation is still only ~0.22 because **the Player's own team over-performs at weak tours regardless of the Player's card** — a fresh 1.5★ side finishes ~1st on average at Club (wins ~30%), and tested player cards 11/8/**6** all win ~25-30% (lowering the hero does NOT help). The boost is structural: the Player's matches run the gap-filled **roster path** (team topped to a standard total) while most games run the **derived scalar path** scaled down at weak tours — so the Player's side is too strong there (and finishes last at strong tours). This is the same root as the early-game build-equality gap (WS5). **Next rung:** reduce one Player's leverage on team results / reconcile roster-vs-derived scaling at weak tours. Nico's call (2026-06-14): merge the solid wins (scale + display + pacing + proportional noise), fix this structurally next.

**Other observed:** WS5 build-equality at low totals (sum 44 spread 13.7 → heals to 1.9 by 125) folds into the same structural rung. Joker/economy/scoring ledger untouched by construction; env in band.
